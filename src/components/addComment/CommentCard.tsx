                ...
                <img className="w-10 h-10 mt-2 rounded-2xl border border-gray-100  " src={avatarSrc}/>
                    <div className="flex items-center">
                        <div className="ml-1.5 text-sm leading-tight">
                            <span className="text-black  font-bold block ">{comment.authorName}</span>
                            <span className="text-gray-500  font-normal block">@{comment.authorEmail}</span>
                        </div>
                    </div>  
                </div>

                <span>
                    <p className="text-black   leading-snug mt-3 ml-2">{comment.comment}</p>
                    <p className="text-gray-500   py-1 my-0.5 ml-2 font-sans">{formatTweetDate(comment.createdAt.toString())}</p>
